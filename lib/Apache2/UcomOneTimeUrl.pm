...
#       return $class->intruder($r, $stuff);
 ...
 sub deliver {
     ...
     my $stuff = $o{$key};
     my $subr = $r->internal_redirect($stuff->{uri});
...
<Location /contents>
  ...
  PerlSetVar OneTimeUri ../webm/hoge.webm
</Location>
...
print Dumper("db:".$db);
print "http://fuga/contents/", Apache2::UcomOneTimeUrl->authorize($db, $comments), ".webm\n";

Now:

    #hogehogefugafuga is any word
    % authorize.pl foo
    http://fuga/contents/2c61de78edd612cf79c0d73a3c7c94fb
